...
| --- | --- | ---: | --- |
| [CasaDrop](Apps/CasaDrop/) | 2.5.0 | 8086 | 2026-09-14 on ZimaOS v1.7.1: 2.5.0 adds an activity log, resumable uploads and session management, and fixes email-share (the dialog never sent a sender, which the server wrongly required). The multi-arch image was built and pushed for this release; both per-architecture digests the store pins match Docker Hub's manifest list, and the amd64 digest matches the image verified before the push — run in a container it was `healthy`, the no-sender email payload passed validation (no longer the old 400), and `/api/events` and `/api/sessions` answered 200. Before that, 2.4.4 on ZimaOS v1.7.1 through the install API as an isolated copy: `running` after 6 s, `healthy`, 0 restarts, root read-only, five host folders `rw=false`, `/DATA/AppData` not mounted, `/healthz` `ok`, the setup page rendered in a real browser, one setup token in the log; removed afterwards, the shared image surviving the uninstall because Docker refuses to delete an image a running container uses |
