...
| --- | --- | ---: | --- |
| [CasaDrop](Apps/CasaDrop/) | 2.5.1 | 8086 | 2026-09-15: 2.5.1 makes SMTP encryption selectable (STARTTLS 587 / SSL-TLS 465 / None); the settings form previously always sent STARTTLS, so a provider that requires implicit TLS on port 465 could not be used. The fix was verified end-to-end in a real browser (German UI) against a binary built from this commit: choosing SSL/TLS stored `use_tls` with port 465, STARTTLS stored `use_starttls` with port 587, each confirmed through the SMTP config API and the SQLite row. The multi-arch image (tag 2.5.1) resolves on Docker Hub to manifest list `sha256:ea7bb29a…` with amd64 `sha256:7a60ecbe…` and arm64 `sha256:a19299b6…`, confirmed by `imagetools inspect`. The 2.5.1 image itself was not re-run on a ZimaOS host — the change is frontend-only on top of 2.5.0. 2.5.0 (2026-09-14, ZimaOS v1.7.1) added the activity log, resumable uploads and session management and was host-verified: run in a container it was `healthy`, `/api/events` and `/api/sessions` answered 200. Before that, 2.4.4 on ZimaOS v1.7.1 through the install API as an isolated copy: `running` after 6 s, `healthy`, 0 restarts, root read-only, five host folders `rw=false`, `/DATA/AppData` not mounted, `/healthz` `ok`, the setup page rendered in a real browser, one setup token in the log; removed afterwards, the shared image surviving the uninstall because Docker refuses to delete an image a running container uses |
...
| [MediaCMS](Apps/MediaCMS/) | 8.3.4 | 8010 | 2026-08-19 on ZimaOS v1.7.1-beta1: six containers from this file (only the four documented values changed), db and redis `healthy`, `migrations` exited 0, and the portal signed into **in a real browser** — "Erfolgreich als admin@localhost angemeldet", superuser menus rendered. The reason this entry exists in a usable state at all is a fault that no status check sees: the bundled nginx forwards the Host header without the port, so on any published port other than 80 everything Django derives from it is wrong. That broke three things at once — every POST answered 403 (no login, no upload, no comment), the frontend shipped a portless API base so its XHRs went to port 80 and were refused by CORS, and the canonical link pointed at the wrong server. Verifying the login with `curl` passes, because curl sends no `Origin` header; it only failed on the first real click. Repaired at the cause, with the control that a **foreign** origin still gets 403. Two more things upstream's compose supplies through a bind mount of its own git checkout, which no packaged install has: the Django `SECRET_KEY` (otherwise regenerated per container and per recreate, logging everyone out on each update) and the default avatar `userlogos/user.jpg`, which `.dockerignore` keeps out of the image — both handled here. `/static` deliberately has no mount: measured, a bind leaves 0 files there where the image has 374 (68 MB), which would serve the portal as unstyled HTML while every container reports healthy. Then the whole point, end to end: a 90-second clip uploaded through the browser and played back. Read out of the running installation afterwards rather than assumed — `encoding_status: success`, **six renditions** (144p through 1080p) each `success` at `progress: 100`, plus HLS with 13 playlists and 138 segments, 165 files and 25 MB sitting in the bind mount owned by `www-data`. The view counter stands at 4, and it is incremented by exactly the `…/action/watch` call that CORS was refusing before the fix. **amd64 only** — upstream publishes no arm64 image, so this will not run on a ZimaBoard. Not verified: behaviour behind an HTTPS reverse proxy, and a second installation on a different host |
