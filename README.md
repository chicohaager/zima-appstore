...
| --- | --- | ---: | --- |
| [CasaDrop](Apps/CasaDrop/) | 2.4.4 | 8086 | 2026-09-14 on ZimaOS v1.7.1: installed through the install API from this file as an isolated copy (name, published port and data directory renamed, nothing else), because the host already ran the maintainer's own CasaDrop 2.4.4 on 8086 — `running` after 6 s, `healthy`, 0 restarts, root filesystem read-only, the five host folders mounted `rw=false` and `/DATA/AppData` not among them, `/healthz` returning `ok`, `/` redirecting to the setup page, which rendered in a real browser, exactly one setup token in the log. The image ID on the host matches Docker Hub's digest for tag 2.4.4, and the per-architecture digests the store build pins match Docker Hub's manifest list. Removed again afterwards — container, data directory, network and port all gone; the shared image survived the uninstall because Docker refuses to delete an image a running container uses. Before that, 2.4.2 on 2026-07-26 (ZimaOS v1.7.0-beta1): installed from this exact file on a machine that had never run it, `healthy` after 14 s, tile in the grid with the right logo, mounts checked one by one |
